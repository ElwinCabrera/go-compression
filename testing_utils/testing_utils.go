package testingutils

import (
	"bytes"
	"fmt"
	"math"
	"math/rand"
)

func GetSomeTestData() [][]byte {

	//Some Sensible scenarios
	t0 := []byte("A_DEAD_DAD_CEDED_A_BAD_BABE_A_BEADED_ABACA_BED")
	t1 := []byte{'A', ' ', 'S', 'A', 'D', ' ', 'S', 'A', 'L', 'A', 'D'}
	t2 := []byte("")
	//
	////Edge cases
	t3 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(50000000, 2) // array only contains two types of elements 0 or 1
	t4 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(50000000, 1) // array only contains one element 0

	//
	////Test complete randomness with a worst case scenario where we have to pick 1/256
	t5 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(5000, 256)
	t6 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(50000000, 256)

	//Simulation of english alphabet (this is unfair since most of the time it will end up with uniform probability, but good to test what is the worst our program can do)
	//In reality we know the most languages have structure i.e punctuation symbols don't have the same probability of occurring as the letter E or A
	t7 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(50000000, 26)        // english alphabet 26 letters
	t8 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(50000000, 26+16)     // english alphabet(upper or lower case but not both) + punctuation symbols = 26 + 16 = 42
	t9 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(50000000, 26+16+10)  // english alphabet(upper or lower case but not both), punctuation, and typographical symbols = 26 + 16 + 10 = 52 (also tests 26 lowercase and 26 uppercase = 52)
	t10 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(50000000, 52+16)    // english alphabet(lowercase + uppercase) + punctuation symbols = 26 + 26 + 16 = 68
	t11 := GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(50000000, 52+16+10) // english alphabet(lowercase + uppercase), punctuation, and typographical symbols = 26 + 26 + 16 + 10 = 78

	//return [][]byte{t8}
	return [][]byte{t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11}
}

// This function is too random if we pick a huge array size then we will reach uniform probability which is the worst the algorithm can do
// in real life data such as the english language are less random and have some sort of structure.
// However, randomness is sometimes good when stress testing the system
func GetByteArrayOfSizeXOfRandomBytesWithMaxRandomByteValue(arraySize int, maxByteValue int) []byte {
	var buf bytes.Buffer
	for i := 0; i < arraySize; i++ {
		ranByte := byte(rand.Intn(maxByteValue))
		buf.WriteByte(ranByte)
	}
	return buf.Bytes()
}

//mainly for debug

func printFracRepInt16(fracRepInt uint16) {
	actualValue := GetFloatFromFractionalRepresentationInt16(fracRepInt)
	fmt.Printf("Float Num: %v\n", actualValue)
}

func printFracRepInt32(fracRepInt uint32) {
	actualValue := GetFloatFromFractionalRepresentationInt32(fracRepInt)
	fmt.Printf("Float Num: %v\n", actualValue)
}

func GetFloatFromFractionalRepresentationInt16(fracRepInt uint16) float64 {
	actualNum := 0.0

	i := 1
	bit := uint16(0)
	for ; i < 16; i++ {
		bit = (fracRepInt & 0x8000) >> 15
		fracRepInt <<= 1
		actualNum += float64(bit) * math.Pow(2.0, -1.0*float64(i))
	}
	for ; i < 32; i++ {
		actualNum += float64(bit) * math.Pow(2.0, -1.0*float64(i))
	}
	return actualNum
}

func GetFloatFromFractionalRepresentationInt32(fracRepInt uint32) float64 {
	actualNum := 0.0

	i := 1
	bit := uint32(0)
	for ; i < 32; i++ {
		bit = (fracRepInt & 0x80000000) >> 31
		fracRepInt <<= 1
		actualNum += float64(bit) * math.Pow(2.0, -1.0*float64(i))
	}
	for ; i < 64; i++ {
		actualNum += float64(bit) * math.Pow(2.0, -1.0*float64(i))
	}
	return actualNum
}
