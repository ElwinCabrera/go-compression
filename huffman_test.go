...
func TestAll(t *testing.T) {
	...
	testStr3 := createRandomStringOfASCIIChars(5000)
	testStr4 := createRandomStringOfASCIIChars(500000)
	testDataToCompress := []string{testStr1, testStr2, testStr3, testStr4}

	for i, str := range testDataToCompress {
		fmt.Printf("Testing string at index %v ... ", i)
		data := []byte(str)
	...
	veryLongTestStr := ""
	for i := 0; i <= 20; i++ {
		veryLongTestStr += testStr4
	}

	data := []byte(veryLongTestStr)
	testSerializeDeSerializeOfHuffmanCodes(t, &data)
	testCompressAndDecompress(t, &data)
