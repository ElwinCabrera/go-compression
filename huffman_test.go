...
func testSerializeDeSerializeOfHuffmanCodes(t *testing.T, testData *[]byte) {
	...
	probabilityOfEachSymbolOccurring := 1 / float64(256)
	inverseP := 1 / probabilityOfEachSymbolOccurring
	predictedEntropy := probabilityOfEachSymbolOccurring * math.Log2(inverseP) * float64(len(freqMap))
	...
	fmt.Printf("\n\tData size is %v containing %v unique symbols and each symbol can be any number between 0-255 (256 total)\n", len(*testData), len(freqMap))
	fmt.Printf("\tPredicted entropy: %f bits/sym\n", predictedEntropy)
	fmt.Printf("\tActual entropy %f bits/sym\n", actualEntropy)
	...
	fmt.Printf("\tActual compressed size (not including serialized huffman codes and extra 1 byte): %v bytes\n", compressedDataSize)
	...
	fmt.Printf("\tEfficiency %f percent\n", (predictedEntropy/actualEntropy)*100)

	fmt.Printf("\tSerialized huffman code size: %v bytes\n", len(serializedHuffmanCodes))
	fmt.Printf("\tTotal compressed size: %v bytes\n", len(compressedData))

}

// This will have very high entropy
func getRandomStingOfXBytesBetween0x00and0xFF(size int) []byte {
...
//func getRandomStingOfEnglishAlphabetAndSymbols(size int) []byte {
//
//}

func getByteArrayOfWithSameValueRepeated(size int) []byte {

	var buf bytes.Buffer
	randInt := rand.Intn(256)
	for i := 0; i < size; i++ {
		buf.WriteByte(uint8(randInt))
	}
	return buf.Bytes()
}

func getByteArrayOfWithTwoValueRepeating(size int) []byte {

	var buf bytes.Buffer
	randInt1 := rand.Intn(256)
	randInt2 := rand.Intn(256)
	i := 0
	if size%2 != 0 {
		buf.WriteByte(uint8(randInt1))
		i = 1
	}
	for ; i < size; i++ {
		buf.WriteByte(uint8(randInt1))
		buf.WriteByte(uint8(randInt2))
	}
	return buf.Bytes()
}

func getBytes0x00To0xFFRepeatedXTimes(size int) []byte {
	var buf bytes.Buffer
	j := 0
	for i := 0; i < size; i++ {
		for j = 0; j <= 255 && buf.Len() < size; j++ {

			buf.WriteByte(uint8(j))
		}
	}
	j = 0
